...
const EditOrders = () => {
  ...
  // console.log('dbimageis',dbimage)
  const datas = useRoute().params.id;
  ...
  // console.log('audiois',recordTime)
  ...
  const [weight1, setWeight1] = useState('hi');
  ...
  console.log('meltingpoint', meltingpointdata.indexOf(meltingpoint));
    ...
    try {
      ...
      AudioRecorder.onFinished = (error, filePath) => {
        if (error) {
          ...
          Toast.show('Audio Recorded Successfully');
        } else {
    ...
    try {
      ...
      if (imagePath.length > 0) {
        imagePath.map((item, index) => {
          ...
          console.log('imagepathlen',item)
        });
      } else {
        // data.append('catalog_images', dbimage ? dbimage[0] : modalImage);
        data.append('catalog_images', modalImage ? modalImage:dbimage[0]);
        // console.log('imagepathfal',imagePath)
        // console.log('dbimageis',dbimage)
        // console.log('dbimageis2',modalImage)
      }
      if (audio64) {
      ...
      data.append('order_id', editData.order_id);
        ...
        await fetch(endpoint, {
          ...
          .then(result => {
            ...
            if (result.success === false) {
              ...
              setLoading(false);
              Alert.alert('Order Edited Successfully');
              // console.log('editedOrder', data)
    ...
    <ScrollView
          ...
          {catalog ? (
            <ModalComponenet
              ...
              onImagePress={(item, index) => {
                ...
                console.log('modalimage',item.image_url);
                setVisible(false);
              }}
              onPress={(item, index) => {
      ...
      <View style={{alignItems: 'center'}}>
        ...
        {/* <ScrollPicker
          ...
          selectedIndex={meltingpointdata.indexOf(meltingpoint)}
          renderItem={(data, index) => {
            return (
              <View style={styles.pickcontainer}>
...
<View style={{...styles.headingContainer}}>
      ...
      {loading ? (
        ...
        <TouchableOpacity
          ...
          <Paragraph style={styles.enquireText}>Update Order</Paragraph>
        </TouchableOpacity>
      )}
    </ScrollView>
  );
